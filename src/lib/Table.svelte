<script lang="ts">
	...
	/**
	 * specifies the name of a column and the data it holds (`getValue` receives an item in `data`);
	 * optionally, define a class name that allows to address the column via CSS
	 */
	...
	<thead>
		<tr>
			{#each columns as { name, class: className }}
				<th class={className} class:highlight={highlightFirstColumn} scope="col">
					{name}
				</th>
			{/each}
		</tr>
...
<style>
	...
	thead tr th:first-of-type.highlight {
		text-align: left;
	}
